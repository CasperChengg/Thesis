...
// template<typename T>
// float CalculateDiversity(std::vector<std::vector<T>> &dataset, bool is_data_exists[], uint32_t n_classes);
...
void Proposed(std::vector<std::vector<T>>dataset, uint32_t n_classes, uint32_t k)
{
    ...
    for(uint32_t n_iterations = 0;;){
        ...
        {
            // return CalculateDiversity(dataset, is_existing_data, n_classes);
        }
        else
        {
...
//         float diversity = CalculateDiversity(dataset, is_existing_data, n_classes);
...
// float CalculateDiversity(std::vector<std::vector<T>> &dataset, std::bitset<N> &is_existing_data, uint32_t n_classes)
// {
...
//     {
//         if(!is_existing_data.test(data_idx))
//         {
//             continue;
//         }
//         uint32_t data_label = dataset[data_idx][data_label_idx];
